...
export default class FilePage {
    constructor() {
        ...
        this.confirmationThatFileWasDeleted = Selector('h2').withExactText('Shot will be deleted momentarily.')
        this.pendingPublishing = Selector('.upload-publishing-title')
        
        this.checkThatFileWasUploaded = async (title, description) => {
            await t
                .expect(this.pendingPublishing.exists).ok()
                .expect(this.imageTitle.innerText).contains(title)
                .expect(this.imageDescription.innerText).contains(description)
                .expect(this.uploadedImage.exists).ok()
        }
